...
do
  eni=$(aws ec2 create-network-interface --region "$region" --subnet-id "$subnet_id" --groups "$groups")
...
curl "${deploy_lambda}"  -H 'Content-Type: application/json' --fail -d "{\"vm\": \"$instance_id\"}" > /tmp/deploy.sh
