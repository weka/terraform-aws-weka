...
not_ready_hosts=$(weka ${protocol} cluster status | grep 'Not Ready' | wc -l)
all_hosts=$(weka ${protocol} cluster status | grep 'Host' | wc -l)

function check_s3_cluster_status() {
  if (( all_hosts > 0 && not_ready_hosts == 0 && all_hosts == cluster_size )); then
          echo "$(date -u): ${protocol} cluster is already created"
          weka ${protocol} cluster status
          exit 0
  fi
}
