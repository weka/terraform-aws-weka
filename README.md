...
| <a name="input_vm_username"></a> [vm\_username](#input\_vm\_username) | For display purposes only while using custom AMI. This variable will not change or create a user with the specified name. This is only to ensure the outputs are correct if your AMI has a different user configured. If you are not using a custom AMI the user will be ec2-user | `string` | `"ec2-user"` | no |
...
| <a name="output_vm_username"></a> [vm\_username](#output\_vm\_username) | For display purposes only while using custom AMI. This variable will not change or create a user with the specified name. This is only to ensure the outputs are correct if your AMI has a different user configured. If you are not using a custom AMI the user will be ec2-user |
