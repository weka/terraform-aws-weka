...
No special upgrade steps are needed: a plain `terraform apply` replaces the attachment in place.
...
After upgrading, confirm the expected policies are attached:
...
| <a name="module_self_signed_certificate"></a> [self\_signed\_certificate](#module\_self\_signed\_certificate) | ./modules/self_signed_certificate | n/a |
...
| <a name="input_ami_id"></a> [ami\_id](#input\_ami\_id) | AMI ID to use for backend instances. If not specified, automatically selects: Amazon Linux 2023 x86\_64 for i3en instances, Amazon Linux 2023 ARM for i8ge instances. | `string` | `null` | no |
...
| <a name="input_client_instance_ami_id"></a> [client\_instance\_ami\_id](#input\_client\_instance\_ami\_id) | The default AMI ID is set to Amazon Linux 2023. For the list of all supported Weka Client OSs please refer to: https://docs.weka.io/planning-and-installation/prerequisites-and-compatibility#operating-system | `string` | `null` | no |
...
| <a name="input_clients_custom_data_post_mount"></a> [clients\_custom\_data\_post\_mount](#input\_clients\_custom\_data\_post\_mount) | Custom data to pass to the instances, will run after weka agent install and mount. | `string` | `""` | no |
| <a name="input_clients_custom_data_pre_mount"></a> [clients\_custom\_data\_pre\_mount](#input\_clients\_custom\_data\_pre\_mount) | Custom data to pass to the instances, will run before weka agent install and mount. | `string` | `""` | no |
...
| <a name="input_clients_weka_cgroups_mode"></a> [clients\_weka\_cgroups\_mode](#input\_clients\_weka\_cgroups\_mode) | Weka cgroups mode for client instances. Valid values are 'auto' and 'force\_v2'. | `string` | `"force_v2"` | no |
...
| <a name="input_containers_config_map"></a> [containers\_config\_map](#input\_containers\_config\_map) | Maps the number of objects and memory size per machine type. | <pre>map(object({<br>    compute  = number<br>    drive    = number<br>    frontend = number<br>    nvme     = number<br>    nics     = number<br>    memory   = list(string)<br>  }))</pre> | <pre>{<br>  "i3en.12xlarge": {<br>    "compute": 4,<br>    "drive": 2,<br>    "frontend": 1,<br>    "memory": [<br>      "310.7GB",<br>      "310.4GB"<br>    ],<br>    "nics": 8,<br>    "nvme": 4<br>  },<br>  "i3en.24xlarge": {<br>    "compute": 9,<br>    "drive": 4,<br>    "frontend": 1,<br>    "memory": [<br>      "384GB",<br>      "384GB"<br>    ],<br>    "nics": 15,<br>    "nvme": 8<br>  },<br>  "i3en.2xlarge": {<br>    "compute": 1,<br>    "drive": 1,<br>    "frontend": 1,<br>    "memory": [<br>      "32.9GB",<br>      "32.64GB"<br>    ],<br>    "nics": 4,<br>    "nvme": 2<br>  },<br>  "i3en.3xlarge": {<br>    "compute": 1,<br>    "drive": 1,<br>    "frontend": 1,<br>    "memory": [<br>      "62GB",<br>      "61.7GB"<br>    ],<br>    "nics": 4,<br>    "nvme": 1<br>  },<br>  "i3en.6xlarge": {<br>    "compute": 4,<br>    "drive": 2,<br>    "frontend": 1,<br>    "memory": [<br>      "136.5GB",<br>      "136.2GB"<br>    ],<br>    "nics": 8,<br>    "nvme": 2<br>  },<br>  "i8ge.12xlarge": {<br>    "compute": 6,<br>    "drive": 4,<br>    "frontend": 1,<br>    "memory": [<br>      "310.7GB",<br>      "310.4GB"<br>    ],<br>    "nics": 12,<br>    "nvme": 4<br>  },<br>  "i8ge.18xlarge": {<br>    "compute": 11,<br>    "drive": 3,<br>    "frontend": 1,<br>    "memory": [<br>      "347GB",<br>      "347GB"<br>    ],<br>    "nics": 16,<br>    "nvme": 6<br>  },<br>  "i8ge.24xlarge": {<br>    "compute": 10,<br>    "drive": 4,<br>    "frontend": 1,<br>    "memory": [<br>      "384GB",<br>      "384GB"<br>    ],<br>    "nics": 16,<br>    "nvme": 8<br>  },<br>  "i8ge.2xlarge": {<br>    "compute": 1,<br>    "drive": 1,<br>    "frontend": 1,<br>    "memory": [<br>      "32.9GB",<br>      "32.64GB"<br>    ],<br>    "nics": 4,<br>    "nvme": 2<br>  },<br>  "i8ge.3xlarge": {<br>    "compute": 3,<br>    "drive": 1,<br>    "frontend": 1,<br>    "memory": [<br>      "62GB",<br>      "61.7GB"<br>    ],<br>    "nics": 6,<br>    "nvme": 1<br>  },<br>  "i8ge.48xlarge": {<br>    "compute": 14,<br>    "drive": 8,<br>    "frontend": 1,<br>    "memory": [<br>      "384GB",<br>      "384GB"<br>    ],<br>    "nics": 24,<br>    "nvme": 16<br>  },<br>  "i8ge.6xlarge": {<br>    "compute": 6,<br>    "drive": 2,<br>    "frontend": 1,<br>    "memory": [<br>      "136.5GB",<br>      "136.2GB"<br>    ],<br>    "nics": 10,<br>    "nvme": 2<br>  }<br>}</pre> | no |
...
| <a name="input_data_services_instance_ami_id"></a> [data\_services\_instance\_ami\_id](#input\_data\_services\_instance\_ami\_id) | AMI ID to use, Amazon Linux 2023 is the supported OS. | `string` | `null` | no |
...
| <a name="input_data_services_weka_cgroups_mode"></a> [data\_services\_weka\_cgroups\_mode](#input\_data\_services\_weka\_cgroups\_mode) | Weka cgroups mode, valid values are 'auto' and 'force\_v2' | `string` | `"force_v2"` | no |
...
| <a name="input_hotspare"></a> [hotspare](#input\_hotspare) | Number of hotspares to set on weka cluster. Refer to https://docs.weka.io/weka-system-overview/cluster-capacity-and-redundancy-management#hot-spare-capacity | `number` | `1` | no |
...
| <a name="input_lambdas_version"></a> [lambdas\_version](#input\_lambdas\_version) | Lambdas code version (hash) | `string` | `"55e700ea19c72068c389e563f66ba0c0"` | no |
...
| <a name="input_nfs_protocol_gateway_instance_ami_id"></a> [nfs\_protocol\_gateway\_instance\_ami\_id](#input\_nfs\_protocol\_gateway\_instance\_ami\_id) | AMI ID to use, Amazon Linux 2023 is the supported OS. | `string` | `null` | no |
...
| <a name="input_s3_protocol_gateway_instance_ami_id"></a> [s3\_protocol\_gateway\_instance\_ami\_id](#input\_s3\_protocol\_gateway\_instance\_ami\_id) | AMI ID to use, Amazon Linux 2023 is the supported OS. | `string` | `null` | no |
...
| <a name="input_sg_custom_ingress_rules"></a> [sg\_custom\_ingress\_rules](#input\_sg\_custom\_ingress\_rules) | Custom inbound rules to be added to the security group. | <pre>list(object({<br>    from_port   = number<br>    to_port     = number<br>    protocol    = string<br>    cidr_blocks = list(string)<br>  }))</pre> | `[]` | no |
...
| <a name="input_smb_protocol_gateway_instance_ami_id"></a> [smb\_protocol\_gateway\_instance\_ami\_id](#input\_smb\_protocol\_gateway\_instance\_ami\_id) | AMI ID to use, Amazon Linux 2023 is the supported OS. | `string` | `null` | no |
...
| <a name="input_tags_map"></a> [tags\_map](#input\_tags\_map) | A map of tags to assign the same metadata to all resources in the environment. Format: key:value. Note: Manually tagged resources will be overridden by Terraform apply. Example: if we want to have 2 tags, one with key "Key1" and value "Val1" and another one with key "Key2" and value "Val2" , we should set: tags\_map = { "Key1" = "Val1", "Key2" = "Val2" } | `map(string)` | `{}` | no |
...
| <a name="input_tiering_obs_iam_bucket_prefix"></a> [tiering\_obs\_iam\_bucket\_prefix](#input\_tiering\_obs\_iam\_bucket\_prefix) | The prefix for the IAM policy that will be created for the OBS bucket. This prefix will be used to create a unique policy rule. | `string` | `""` | no |
...
| <a name="input_weka_cgroups_mode"></a> [weka\_cgroups\_mode](#input\_weka\_cgroups\_mode) | Weka cgroups mode for backend instances. Valid values are 'auto' and 'force\_v2'. Defaults to 'force\_v2'. | `string` | `"force_v2"` | no |
...
| <a name="output_nfs_protocol_gateways_name"></a> [nfs\_protocol\_gateways\_name](#output\_nfs\_protocol\_gateways\_name) | Name of NFS protocol gateway instances |
...
| <a name="output_s3_protocol_gateways_name"></a> [s3\_protocol\_gateways\_name](#output\_s3\_protocol\_gateways\_name) | Name of S3 protocol gateway instances |
