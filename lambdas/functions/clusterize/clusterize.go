...
func doClusterize(p ClusterizationParams, funcDef functions_def.FunctionDef) (clusterizeScript string, err error) {
	...
	instances, err := common.AddInstanceToStateInstances(p.StateTable, p.StateTableHashKey, stateKey, p.Vm)
	if err != nil {
		log.Error().Err(err).Send()
		return
	}
	...
	msg := fmt.Sprintf("This (%s) is instance %d/%d that is ready for clusterization", p.Vm.Name, len(instances), initialSize)
	log.Info().Msgf(msg)
	...
	instancesNames := common.GetInstancesNames(instances)
...
func doNFSClusterize(p ClusterizationParams, funcDef functions_def.FunctionDef) (clusterizeScript string, err error) {
	...
	msg := fmt.Sprintf("This (%s) is nfs instance %d/%d that is ready for joining the interface group", p.Vm.Name, len(instances), initialSize)
	log.Info().Msgf(msg)
	if len(instances) != initialSize {
		clusterizeScript = cloudCommon.GetScriptWithReport(msg, funcDef.GetFunctionCmdDefinition(functions_def.Report))
		return
