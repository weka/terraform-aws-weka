...
func AddInstanceToStateInstances(table, hashKey, stateKey string, newInstance protocol.Vm) (instances []protocol.Vm, err error) {
	err = LockState(table, hashKey, stateKey)
	if err != nil {
		return
	}

	instances, err = addInstanceToStateInstances(table, hashKey, stateKey, newInstance)
	if err != nil {
		log.Error().Err(err).Send()
	}
